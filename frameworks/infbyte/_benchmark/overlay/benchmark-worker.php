...
$environmentResponse = static function (string $path) use ($driver): ?Response {
...
$handle = static function (Request $request) use ($application, $environmentResponse): Response {
    $path = $request->getUri()->getPath();
    $environment = $environmentResponse($path);
...
if ($driver === 'frankenphp') {
    $callback = static function () use ($handle): void {
        ...
        (new DefaultEmitter())->emit($handle($request));
    };
